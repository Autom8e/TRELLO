...
public class BoardPage extends BasePage{
    ...
    public List<WebElement> defaultThreeListHeader;

    @FindBy(xpath = "(//span[@class='js-add-a-card'])[1]")
    public WebElement addCartForFirstList;

    @FindBy(xpath = "(//span[@class='js-add-a-card'])[2]")
    public WebElement addCartForSecondList;

    @FindBy(xpath = "(//span[@class='js-add-a-card'])[3]")
    public WebElement addCartForThirdList;
    ...
    // bu metodu list name i edit etmek için kullan
    public WebElement listPicker(String listName){
        return Driver.get().findElement(By.xpath("//textarea[@aria-label='"+listName+"']"));
    }
    /*

    //span[@class='list-card-title js-card-name' and contains(.,'logo')]

    //textarea[@aria-label='reviewed']

    //div[@class='list-card-details u-clearfix'] ----> card header

    //input[@value='Add card'] ---->add cart btn

    //a[@class='icon-lg icon-close dark-hover js-cancel'] ---< x buton

    (//div[@class='list js-list-content'])[3]/div[2]/a ----> list içersindeki cardları bulmak için ***Çağlar*** bunu beraber bulmuştuk
...
//span[@class='card-label mod-selectable card-label-red  js-select-label'] ---<label picker
